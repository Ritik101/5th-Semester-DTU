...
function updateTable(processes) {
  ...
  for (const key in processes) {
    if (processes.hasOwnProperty(key)) {
      ...
      document.getElementById(key + "-turnaround").innerHTML =
        element.turnaround;
      document.getElementById(key + "-completion").innerHTML =
        element.completion;
  ...
  document.getElementById("averageTime").innerHTML = `Average waiting time : ${(
    waitingTime / count
  ).toFixed(2)} unit time<br>Average Turnaround time: ${(
...
function addRow() {
  if (
    !document.getElementById("addRowBtn").classList.contains("simulation-done")
  ) {
...
function removeRow() {
  if (
    !document
      ...
      .classList.contains("simulation-done")
  ) {
    let element = document.getElementById("table");
    let noOfRows = element.rows.length;
    if (noOfRows > 3) {
